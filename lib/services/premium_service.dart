...
class PremiumService {
  ...
  Future<bool> activatePremiumPlan(PremiumPlan plan) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      debugPrint('PremiumService: Activated $plan plan');
      return true;
    } catch (e) {
      debugPrint('PremiumService: Failed to activate plan - $e');
      return false;
    }
  }

  /// Handle successful in-app purchase
  Future<void> handleSuccessfulPurchase(String productId) async {
    try {
      PremiumPlan plan;

      switch (productId) {
        case 'biztracker_monthly_premium':
          plan = PremiumPlan.monthly;
          break;
        case 'biztracker_annual_premium':
          plan = PremiumPlan.annual;
          break;
        case 'biztracker_lifetime_premium':
          plan = PremiumPlan.lifetime;
          break;
        default:
          debugPrint('PremiumService: Unknown product ID - $productId');
          return;
      }

      await activatePremiumPlan(plan);
      debugPrint('PremiumService: Successfully handled purchase for $plan');
    } catch (e) {
      debugPrint('PremiumService: Failed to handle purchase - $e');
    }
  }
